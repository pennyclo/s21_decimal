...
int s21_is_equal(s21_decimal *dec1, s21_decimal *dec2) {
  ...
  if (decimal_1.data_size == decimal_2.data_size && eq_signs == 0) {
    is_equal = 1;
    for (int i = decimal_1.data_size - 1; i >= 0 && is_equal; i--) {
      uint8_t check1 = CHECK_DEC_BIT(decimal_1.data, i, decimal_1.data_size);
      uint8_t check2 = CHECK_DEC_BIT(decimal_2.data, i, decimal_2.data_size);

      is_equal = (check1 == check2) ? 1 : 0;
    }
  }
...
int s21_is_not_equal(s21_decimal *dec1, s21_decimal *dec2) {
  ...
  if (decimal_1.data_size == decimal_2.data_size && eq_signs == 0) {
    is_not_equal = 0;
    for (int i = decimal_1.data_size - 1; i >= 0 && !is_not_equal; i--) {
      uint8_t check1 = CHECK_DEC_BIT(decimal_1.data, i, decimal_1.data_size);
      uint8_t check2 = CHECK_DEC_BIT(decimal_2.data, i, decimal_2.data_size);

      is_not_equal = (check1 == check2) ? 0 : 1;
    }
  }
